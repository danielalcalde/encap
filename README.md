...
slurm:
  ...
  code:
    ...
    - timeout 23.5h srun --export=ALL,SLURM_RESTARTNR=$SLURM_RESTARTNR bash {run.sh} # Time until the job is terminated by timeout
    - if [ $? -eq 124 ] && [ $SLURM_RESTARTNR -lt 3 ]; then # Number of repetitions
    - sbatch --export=ALL,SLURM_RESTARTNR=$SLURM_RESTARTNR {run.slurm}
    - fi
```
